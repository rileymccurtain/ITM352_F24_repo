...
def load_csv(file_path):
    try:
        ...
        # Display summary
        display_data_summary(sales_data, missing_columns)
...
def display_data_summary(data, missing_columns):
    total_orders = len(data)
    unique_employees = data['employee_id'].nunique()
    unique_regions = data['sales_region'].nunique()
    ...
    unique_product_categories = data['product_category'].nunique() if 'product_category' in data.columns else 0
    unique_states = data['customer_state'].nunique() if 'customer_state' in data.columns else 0
    total_sales_amount = data['unit_price'].sum()
    total_quantities_sold = data['quantity'].sum()

    print("\n--- Data Summary ---")
    print(f"Total Orders: {total_orders}")
    print(f"Number of Unique Employees: {unique_employees}")
    print(f"Number of Sales Regions: {unique_regions}")
...
def total_sales_by_region_and_order_type(data):
    ...
    print(result)
    if confirm_export():
        filename = input("Enter filename for export (without extension): ") + ".xlsx"
...
def confirm_export():
    return input("Do you want to export the results to an Excel file? (yes/no): ").strip().lower() == 'yes'
...
def display_menu(data):
    ...
    # Remove options that cannot be performed due to missing columns
    if 'customer_state' not in data.columns:
        menu_options.remove(menu_options[3])  # Remove Sales by customer type and order type by state
    if 'customer_id' not in data.columns:
        unique_customers = 0
    if 'product_category' not in data.columns:
        menu_options.remove(menu_options[5])  # Remove Total sales quantity and price by customer type
    if 'employee_id' not in data.columns:
        menu_options.remove(menu_options[7])  # Remove Number of unique employees by region
